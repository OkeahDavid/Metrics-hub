...
export async function GET(
  ...
  try {
    ...
    // Use the AnalyticsService to fetch countries data
    const countriesData = await AnalyticsService.getTopCountries(id, fromDate, toDate, limit) as CountryData[];

    return createSuccessResponse(
      countriesData,
      'Countries data retrieved successfully',
      {
        'Cache-Control': 'public, max-age=60' // Cache for 1 minute
      }
    );
  } catch (error) {
    return handleApiError(error, 'Failed to fetch countries data');
  }
}